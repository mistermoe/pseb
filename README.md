...
![Go Badge](https://img.shields.io/badge/Go-00ADD8?logo=go&logoColor=fff&style=flat) [![golangci-lint](https://img.shields.io/badge/golangci--lint-enabled-brightgreen?logo=go&logoColor=fff&style=flat)](https://golangci-lint.run) [![integrity](https://github.com/mistermoe/pseb/actions/workflows/integrity.yml/badge.svg)](https://github.com/mistermoe/pseb/actions/workflows/integrity.yml)
...
Reads a PSEB certificate PDF and returns the holder's 13-digit CNIC (the Pakistani national identity number printed on freelancer certificates). Unlike the data from `ExtractCertificate`, the CNIC is not in the QR code, the JWT, or the verification response — it appears only as printed text, so it's recovered from the PDF's text layer. Matching prefers a value following a `CNIC` label (accepting both `3520122720739` and `35201-2272073-9` forms, normalized to 13 digits) and falls back to any standalone run of exactly 13 digits.

Returns `ErrNoCNIC` if no CNIC can be found. This relies on the certificate having a real text layer (as current PSEB certificates do); it does not OCR image-only scans.
